sap.ui.define([
    ...
    return {
        ...
        POEMState: function (estatus) {
            if (estatus === 'CONFIRMADA' || estatus === 'NOTIFICADA') {
                return "Success";
            }

            else if (estatus === 'NOTIFICADA PARCIAL') {
                return "None";
            }

            else if (estatus === 'FACTURADA PARCIAL') {
                return "Warning";
            } else {
                return "None";
            }
            ...
            /*
                    <div>
                            <span class="badge badge-success" *ngIf="oc.estatus.trim() == 'CONFIRMADA'">{{oc.estatus}}</span>
                            <span class="badge badge-success" *ngIf="oc.estatus.trim() == 'NOTIFICADA'">{{oc.estatus}}</span>
                            <span class="badge badge-secondary"
                              *ngIf="oc.estatus.trim() == 'NOTIFICADA PARCIAL'">{{oc.estatus}}</span>
                            <span class="badge badge-warning" *ngIf="oc.estatus.trim() == 'FACTURADA PARCIAL'">{{oc.estatus}}</span>
                            <h5 class="my-0"><span class="badge badge-light">{{oc.totalOc | currency}} {{oc.moneda}}</span>
                            </h5>
        ...
        enableQuantityField: function (motivo) {
            console.log(motivo)

            if (motivo == "Solicitud de modificación de cantidad de reparto") {
                return true
            } else {
                return false
            }
        ...
        enableDateField: function (motivo) {

            if (motivo == "Solicitud de modificación de fecha de entrega") {
                return true
            } else {
                return false
            }
